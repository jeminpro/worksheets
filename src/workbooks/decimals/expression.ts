import type { DecimalsFormat, DecimalsProblem } from "./types";

export type DecimalTextSegment =
  | { kind: "text"; value: string }
  | { kind: "fraction"; whole?: string; numerator: string; denominator: string };

export function getFormat(problem: DecimalsProblem): DecimalsFormat {
  if (problem.format) return problem.format;
  return problem.given === undefined ? "text" : "values";
}

export function getGiven(problem: DecimalsProblem): readonly string[] {
...
export function parseDecimalText(text: string): readonly DecimalTextSegment[] {
  const pattern = /(?:(\d+)\s+)?([0-9?a-zA-Z]+)\/([0-9?a-zA-Z]+)/g;
  const segments: DecimalTextSegment[] = [];
  let lastIndex = 0;
...
export function speak(value: string): string {
  return value
    ...
    .replace(/(\d+)m\b/g, "$1 metres")
    .replace(/(\d+)l\b/g, "$1 litres")
    .replace(/\s+/g, " ")
    .trim();
}

export function formatAccessibleQuestion(problem: DecimalsProblem): string {
