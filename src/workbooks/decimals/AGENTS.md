# Decimals assessment workbook

Five A4 11+ assessment pages covering decimal place value, adding & subtracting decimals, multiplying & dividing decimals, converting decimals & fractions, and ordering & comparing decimals. Each page has 15 problems: 5 easy, 5 medium and 5 hard, grouped in labelled bands.

## Files

- `data.ts` — prompts, given values, answers and difficulty for all 75 problems.
- `types.ts` — `DecimalsProblem` / `DecimalsSheet` / topic, difficulty and format unions.
- `expression.ts` — format resolution, decimal & fraction tokenisation and spoken accessibility labels.
- `DecimalText.astro` — mixed number and stacked fraction parser/renderer for fraction conversions.
- `DecimalsSheet.astro` — page chrome and three difficulty bands.
- `DecimalsProblem.astro` — question card, answer reveal and working space.
- Route: `src/pages/topic/decimals/index.astro`.

## Constraints

- Keep five topic pages in this order: decimal place value → adding & subtracting decimals → multiplying & dividing decimals → converting decimals & fractions → order & compare decimals.
...
```powershell
npm run check
npm run check:layout -- --workbook=decimals
```
