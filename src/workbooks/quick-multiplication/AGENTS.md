...
- `data.ts` — `[factor, factor]` pairs and timing strings.
- `types.ts` — `QuickMultiplicationSheet` / `FactorPair` / `QuickMultiplicationTiming`.
- `QuickMultiplicationSheet.astro` — page chrome, timing subheading, 5×8 grid.
- `QuickMultiplicationProblem.astro` — horizontal product; answer is `left × right`.
- Route: `src/pages/topic/quick-multiplication/index.astro`.
...
```powershell
npm run check
npm run check:layout -- --workbook=quick-multiplication
```
